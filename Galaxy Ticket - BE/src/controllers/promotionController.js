...
exports.getAllPromotions = async (req, res) => {
  try {
    ...
    const promotions = await Promotion.find(query).sort({ createdAt: -1 }); // Thêm sort như movie
...
exports.deletePromotion = async (req, res) => {
  try {
    ...
    // Xóa promotion bất kể trạng thái
...
exports.validatePromotionCode = async (req, res) => {
  try {
    ...
    if (promotion.currentUsage >= promotion.maxUsage) {
      ...
      return res
        .status(404)
        .json({ message: "This promotion code has reached its usage limit" });
    }
