  ...
  try {
    ...
    if (movie.showingStatus !== "now-showing") {
      return res.status(400).json({
        success: false,
        message:
          "Chỉ phim đang chiếu (now-showing) mới được phép tạo suất chiếu.",
      });
    }
    ...
    if (await isTimeOverlap(roomId, startTime, endTime)) {
      return res.status(400).json({
        success: false,
        message:
          "Thời gian chiếu bị trùng với suất chiếu khác trong phòng này.",
      });
  ...
  try {
    ...
    res.status(200).json({
      ...
      message: screenings.length
        ...
        : "Không có suất chiếu nào cho rạp này",
      data: screenings,
      count: screenings.length,
    });
  } catch (err) {
