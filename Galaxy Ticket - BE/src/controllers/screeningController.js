...
exports.getScreeningById = async(req, res) => {
    try {
        ...
        const screening = await Screening.findOne(query).populate(
            "movieId roomId createdBy approvedBy"
        );
        if (!screening) {
...
exports.createScreening = async(req, res) => {
    try {
        ...
        // Populate các trường liên kết trước khi lưu vào requestData
        ...
        // Sau khi tạo screening thành công, tạo tự động 8x8 ghế cho screening
        const seatsData = [];
        const rows = 8;
        const cols = 8;
        ...
        await require("../models/Seat").insertMany(seatsData);
...
exports.updateScreening = async(req, res) => {
    try {
        ...
        if (
            (updateData.roomId &&
                updateData.roomId !== screening.roomId.toString()) ||
            (updateData.startTime &&
                updateData.startTime !== screening.startTime.toISOString()) ||
            (updateData.endTime &&
                updateData.endTime !== screening.endTime.toISOString())
        ) {
            if (
                await isTimeOverlap(
                    updateData.roomId || screening.roomId,
                    updateData.startTime || screening.startTime,
                    updateData.endTime || screening.endTime,
                    screening._id
                )
            ) {
                return res.status(400).json({
        ...
        res.status(200).json({
            ...
            message: screening.status === "pending" ?
                "Screening updated successfully" : "Screening update submitted for approval",
...
exports.getScreeningsByTheater = async(req, res) => {
    try {
        const roomsInTheater = await require("../models/Room").find({
        ...
        const screenings = await Screening.find(query).populate(
            "movieId roomId createdBy approvedBy"
        );
        res.status(200).json({
            success: true,
            message: screenings.length ?
                "Lấy danh sách suất chiếu thành công" : "Không có suất chiếu nào cho rạp này",
            data: screenings,
            count: screenings.length,
        });
    } catch (err) {
...
exports.getScreeningsByMovie = async(req, res) => {
    try {
        ...
        const screenings = await Screening.find(query).populate(
            "movieId roomId createdBy approvedBy"
        ); // Loại bỏ theaterId khỏi populate vì Screening không có nó trực tiếp
        if (!screenings.length) {
            return res
                .status(404)
                .json({ message: "Không tìm thấy suất chiếu nào cho phim này" });
        }
        res.json(screenings);
    } catch (err) {
        res.status(500).json({ message: err.message });
